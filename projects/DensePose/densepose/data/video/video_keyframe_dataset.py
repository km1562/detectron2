        ...
        with PathManager.open(video_fpath, "rb") as io:
            ...
            while True:
                try:
                    ...
                    # we then return whatever datas we've already collected
...
def read_keyframes(
    ...
    """
    Reads keyframe datas from a video file.
...
def read_keyframe_helper_data(fpath: str):
    """
    Read keyframe datas from a file in CSV format: the header should contain
    ...
    Args:
        fpath (str): File containing keyframe datas
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(f"Error reading keyframe helper datas from {fpath}: {e}")
        ...
        Returns:
            A dictionary containing two keys:
                images (torch.Tensor): tensor of size [N, H, W, 3] or of size
                    defined by the transform that contains keyframes datas
